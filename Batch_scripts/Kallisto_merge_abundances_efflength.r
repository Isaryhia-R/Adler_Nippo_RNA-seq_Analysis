...
head(thisMat)
counts=thisMat[,c('target_id','eff_length')] # est_counts or tpm or eff_length
head(counts)
...
for(i in 2:length(samples)){
  ...
  counts=as.data.frame(cbind(counts,thisMat[,"eff_length"]))   #est_counts  or tpm or eff_length (CHANGE NAME)
}
...
write.csv(counts,"Nippo_Allsamples_02-13-23_efflength_matrix.csv", quote = FALSE)  ##CHANGE NAME IF TPM or counts!! 
~                                
